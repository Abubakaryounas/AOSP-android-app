...
public class RecipientEditTextView extends MultiAutoCompleteTextView implements
    ...
    private final Drawable mChipBackground = null;
    private final Drawable mChipDelete = null;
    ...
    private final boolean mDragEnabled = false;
    ...
    private final Runnable mDelayedShrink = new Runnable() {
    ...
    private final Set<String> mUntrustedAddresses = new HashSet<>();
    ...
    private final Runnable mHandlePendingChips = new Runnable() {
    ...
    private final String mWarningTextTemplate = "";
...
//        final int startPadding = displayIcon ? mChipTextStartPadding : mChipTextEndPadding;
    ...
    private DrawableRecipientChip constructChipSpan(RecipientEntry contact) {
        ...
        result.setBounds(0, 4, iconWidth, iconHeight);
    ...
    private int calculateOffsetFromBottomToTop(int line) {
        return -(int) ((mChipHeight + (0 * mLineSpacingExtra)) * (Math
                .abs(getLineCount() - line)) + 0);
    }
    ...
    private StateListDrawable constructStateListDeleteDrawable() {
        ...
        if (!mDisableDelete) {
            deleteDrawable.addState(new int[]{android.R.attr.state_activated}, null);
        }
    ...
    public void appendRecipientEntry(final RecipientEntry entry) {
        ...
        if (chip != null) {
            /*removes any unwanted/unfinished text at the chip end*/
            if (getSortedRecipients() != null && getSortedRecipients().length > 0) {
                if (recips != null && recips.length > 0) {
                    final DrawableRecipientChip last = recips[recips.length - 1];
//                    editable.delete(getEditableText().getSpanEnd(last) + 1, getText().length() - 1);
                    editable.delete(getEditableText().getSpanEnd(last), getText().length() - 1);
                }
            } else {
                editable.clear();
            }
            /*inserts the chip at start*/
            getEditableText().insert(chipInsertionPoint, chip);
        }
    }
